...
#define container_of(ptr, ctype, mtype, member) ({		\
	const mtype *__mptr = ptr;				\
	(ctype *) ((char *)__mptr - offsetof(ctype, base));	\
 })

#define obj_to_encoder(ptr) container_of(ptr,			\
					 struct drm_encoder,	\
					 struct drm_mode_object,\
					 base);

#define obj_to_crtc(ptr) container_of(ptr,			\
				      struct drm_crtc,		\
				      struct drm_mode_object,	\
				      base);

#define obj_to_fb(ptr) container_of(ptr,			\
				    struct drm_framebuffer,	\
				    struct drm_mode_object,	\
				    base);

#define obj_to_connector(ptr) container_of(ptr,				\
				           struct drm_connector,	\
				           struct drm_mode_object,	\
				           base);

#define obj_to_property(ptr) container_of(ptr,				\
				          struct drm_property,		\
				          struct drm_mode_object,	\
				          base);

#define obj_to_blob(ptr) container_of(ptr,			\
...
void drm_framebuffer_cleanup(struct drm_framebuffer *fb)
{
	...
	const struct list_head *crtcs = &(dev->mode_config.crtc_list);
...
void drm_crtc_cleanup(struct drm_crtc *crtc)
{
	...
	if (crtc->gamma_store) {
		kmem_free(crtc->gamma_store, sizeof (*(crtc->gamma_store)));
...
void drm_connector_cleanup(struct drm_connector *connector)
{
	...
	list_for_each_safe(list, t, &connector->user_modes) {
		mode = list_entry(list, struct drm_display_mode, &connector->probed_modes);
		drm_mode_remove(connector, mode);
	}
...
struct drm_display_mode *drm_mode_create(struct drm_device *dev)
{
	...
	nmode = kmem_zalloc(sizeof(struct drm_display_mode), KM_SLEEP);
	if (!nmode)
		return NULL;
...
int drm_mode_group_init(struct drm_device *dev, struct drm_mode_group *group)
{
	...
	group->id_list = kmem_zalloc(total_objects * sizeof(uint32_t), KM_SLEEP);
	if (!group->id_list)
		return -ENOMEM;
				     ...
				     struct drm_mode_group *group)
{
	...
	struct drm_connector *connector;
	const struct list_head *crtcs = &(dev->mode_config.crtc_list);
	const struct list_head *encoders = &(dev->mode_config.encoder_list);
	const struct list_head *connectors = &(dev->mode_config.connector_list);
	struct list_head *list;
	int ret;
	...
	list_for_each(list, encoders) {
		encoder = list_entry(list, struct drm_encoder, encoders);
		group->id_list[group->num_crtcs + group->num_encoders++] =  encoder->base.id;
	}

	list_for_each(list, connectors) {
		connector = list_entry(list, struct drm_connector, connectors);
...
void drm_mode_config_cleanup(struct drm_device *dev)
{
	...
	list_for_each_safe(list, t, &dev->mode_config.encoder_list) {
		encoder = list_entry(list, struct drm_encoder, &dev->mode_config.encoder_list);
		encoder->funcs->destroy(encoder);
	}

	list_for_each_safe(list, t, &dev->mode_config.connector_list) {
		connector = list_entry(list, struct drm_connector, &dev->mode_config.connector_list);
		connector->funcs->destroy(connector);
	}

	list_for_each_safe(list, t, &dev->mode_config.property_list) {
		property = list_entry(list, struct drm_property, &dev->mode_config.property_list);
		drm_property_destroy(dev, property);
	}

	list_for_each_safe(list, t, &dev->mode_config.fb_list) {
		fb = list_entry(list, struct drm_framebuffer, &dev->mode_config.fb_list);
		fb->funcs->destroy(fb);
	}

	list_for_each_safe(list, t, &dev->mode_config.crtc_list) {
...
int drm_mode_getresources(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&card_res, (void *)data, sizeof (card_res));
	//TODO
	struct list_head *lh;
	struct drm_framebuffer *fb;
	struct drm_connector *connector;
...
int drm_mode_getcrtc(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&crtc_resp, (void *)data, sizeof (crtc_resp));
...
int drm_mode_getconnector(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&out_resp, (void *)data, sizeof (out_resp));
...
int drm_mode_getencoder(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&enc_resp, (void *)data, sizeof (enc_resp));
...
int drm_mode_setcrtc(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&crtc_req, (void *)data, sizeof (crtc_req));
	...
	struct drm_mode_set set;
	struct list_head *crtcs = &dev->mode_config.crtc_list;
	struct list_head *list;
	...
	if (crtc_req.mode_valid) {
		...
		display_mode = drm_mode_create(dev);
		drm_crtc_convert_umode(display_mode, &crtc_req.mode);
		drm_mode_set_crtcinfo(display_mode, CRTC_INTERLACE_HALVE_V);
	}
...
int drm_mode_cursor_ioctl(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&req, (void *)data, sizeof (req));
	...
	if (req.flags & DRM_MODE_CURSOR_BO) {
		...
		ret = crtc->funcs->cursor_set(crtc, fpriv, req.handle,
...
int drm_mode_addfb(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&r, (void *)data, sizeof (r));
	...
	fb = dev->mode_config.funcs->fb_create(dev, fpriv, &r);
	//TODO
	/*
	if (IS_ERR(fb)) {
...
int drm_mode_rmfb(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&id, (void *)data, sizeof (id));
	...
	list_for_each(list, &fpriv->fbs) {
		fbl = list_entry(list, struct drm_framebuffer, &fpriv->fbs);
		if (fb == fbl)
			found = 1;
	}
...
int drm_mode_getfb(DRM_IOCTL_ARGS)
{
	...
	r.pitch = fb->pitch;
	fb->funcs->create_handle(fb, fpriv, &r.handle);
...
int drm_mode_dirtyfb_ioctl(DRM_IOCTL_ARGS)
{
	...
	if (num_clips && clips_ptr) {
		clips = kmem_zalloc(num_clips * sizeof(*clips), KM_SLEEP);
		if (!clips) {
			ret = -ENOMEM;
			goto out_err1;
		}
	...
	if (fb->funcs->dirty) {
		ret = fb->funcs->dirty(fb, fpriv, flags, r.color,
				       clips, num_clips);
	} else {
		ret = -ENOSYS;
		goto out_err2;
			       ...
			       struct drm_display_mode *mode)
{
	...
	list_for_each_safe(list, t, &connector->user_modes) {
		match_mode = list_entry(list, struct drm_display_mode, &connector->user_modes);
...
int drm_mode_attachmode_ioctl(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&mode_cmd, (void *)data, sizeof (mode_cmd));
	...
	drm_crtc_convert_umode(display_mode, umode);

	ret = drm_mode_attachmode(dev, connector, display_mode);
out:
	mutex_exit(&dev->mode_config.mutex);
	return ret;
}
...
int drm_mode_detachmode_ioctl(DRM_IOCTL_ARGS)
{
	...
	drm_crtc_convert_umode(&display_mode, umode);
	ret = drm_mode_detachmode(dev, connector, &display_mode);
out:
	mutex_exit(&dev->mode_config.mutex);
	return ret;
}
					 ...
					 const char *name, int num_values)
{
	...
	property = kmem_zalloc(sizeof(struct drm_property), KM_SLEEP);
	if (!property)
		return NULL;

	if (num_values) {
		property->values = kmem_zalloc(sizeof(uint64_t)*num_values, KM_SLEEP);
		if (!property->values)
			goto fail;
	}
			  ...
			  uint64_t value, const char *name)
{
	...
	prop_enum = kmem_zalloc(sizeof(struct drm_property_enum), KM_SLEEP);
	if (!prop_enum)
		return -ENOMEM;
...
void drm_property_destroy(struct drm_device *dev, struct drm_property *property)
{
	...
	list_for_each_safe(list, t, &property->enum_blob_list) {
		...
		kmem_free(prop_enum, sizeof (*prop_enum));
	}

	if (property->num_values)
		kmem_free(property->values, sizeof (*(property->values)));
...
{
	DRM_DEVICE;
	struct drm_mode_get_property out_resp;
							  ...
							  void *data)
{
	...
	blob = kmem_zalloc(sizeof(struct drm_property_blob)+length, KM_SLEEP);
	if (!blob)
		return NULL;
...
{
	DRM_DEVICE;
	struct drm_mode_connector_set_property out_resp;
				  ...
				  int gamma_size)
{
	...
	crtc->gamma_store = kmem_zalloc(gamma_size * sizeof(uint16_t) * 3, KM_SLEEP);
	if (!crtc->gamma_store) {
		crtc->gamma_size = 0;
		return 0;
	}
...
int drm_mode_gamma_set_ioctl(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&crtc_lut, (void *)data, sizeof (crtc_lut));
...
int drm_mode_page_flip_ioctl(DRM_IOCTL_ARGS)
{
	...
	DRM_COPYFROM_WITH_RETURN(&page_flip, (void *)data, sizeof (page_flip));
	...
	if (page_flip.flags & DRM_MODE_PAGE_FLIP_EVENT) {
		...
		if (fpriv->event_space < sizeof e->event) {
			//spin_unlock_irqrestore(&dev->event_lock, flags);
			goto out;
		}
		fpriv->event_space -= sizeof e->event;
		//spin_unlock_irqrestore(&dev->event_lock, flags);
...
int drm_mode_create_dumb_ioctl(DRM_IOCTL_ARGS)
{
	...
	if (!dev->driver->dumb_create)
		return -ENOSYS;
	return dev->driver->dumb_create(fpriv, dev, &args);
}
...
int drm_mode_mmap_dumb_ioctl(DRM_IOCTL_ARGS)
{
	...
	return dev->driver->dumb_map_offset(fpriv, dev, args.handle, &args.offset);
}
...
int drm_mode_destroy_dumb_ioctl(DRM_IOCTL_ARGS)
{
	...
	return dev->driver->dumb_destroy(fpriv, dev, args.handle);
}
